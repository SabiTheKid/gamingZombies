    ...
    public static ArrayList<GameObject> createObjectLimits() {
        ...
        for(int y = defaultPadding; y < defaultPadding+Field.height+wallHeight*2; y += wallHeight ) {
            if(y == Field.height/2+defaultPadding){
                gameObjectsList.add(new Door(new Position(defaultPadding,y), new Picture(defaultPadding, y, "entrance_door.png")));
                continue;
            }
            new GameObject(new Position(defaultPadding, y), new Picture(defaultPadding, y, "wall_32.png"));
        }
        //Create right wall
        for(int y = defaultPadding; y < defaultPadding+Field.height+wallHeight*2; y += wallHeight) {
            if(y == Field.height/2+defaultPadding){
                gameObjectsList.add(new Door(new Position(Field.width+padding+defaultPadding, y), new Picture(Field.width+padding+defaultPadding, y, "exit_door.png")));
                continue;
            }
            new GameObject(new Position(defaultPadding+Field.width+padding, y), new Picture(defaultPadding+Field.width+padding, y,"wall_32.png" ));
        }
        //Create up wall
        for(int x = defaultPadding+padding; x < defaultPadding+Field.width+wallWidth; x += wallWidth ) {
            new GameObject(new Position(x, defaultPadding), new Picture(x, defaultPadding, "wall_32.png"));
        }
        //Create down wall
        for(int x = padding+defaultPadding; x < defaultPadding+Field.width+wallWidth; x += wallWidth ) {
            new GameObject(new Position(x, defaultPadding+Field.height+padding), new Picture(x, defaultPadding+Field.height+padding, "wall_32.png"));
        }
